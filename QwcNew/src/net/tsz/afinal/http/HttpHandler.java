...
public class  HttpHandler  <T> extends  AsyncTask<Object, Object, Object> implements EntityCallBack{
	...
	private void makeRequestWithRetries(HttpUriRequest request) throws IOException {
		...
		while (retry) {
			try {
				...
				publishProgress(UPDATE_FAILURE,e,0,"unknownHostException锛歝an't resolve host");
				return;
			} catch (IOException e) {
				cause = e;
				retry = retryHandler.retryRequest(cause, ++executionCount,context);
			} catch (NullPointerException e) {
				// HttpClient 4.0.x 涔嬪墠鐨勪竴涓猙ug
		...
		if(cause!=null)
			...
			throw new IOException("鏈煡缃戠粶閿欒");
	}
	...
	protected Object doInBackground(Object... params) {
		...
		try {
			publishProgress(UPDATE_START); // 寮�
			makeRequestWithRetries((HttpUriRequest)params[0]);
			
		} catch (IOException e) {
			publishProgress(UPDATE_FAILURE,e,0,e.getMessage()); // 缁撴潫
		}
